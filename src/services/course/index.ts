...
export const getUpgradeableCourses = (
  ...
  const purchasedBundlesByCourseId = Object.keys(courses)
    .filter(key => courses[key].courseId === courseId)
    .map(key => {
      const bundleId = courses[key].packageId;
...
export const mergeCourses = (courses: FirebaseCourse) =>
